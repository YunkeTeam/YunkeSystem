...
public class VerifyAspect {
        ...
        if (values.length != 0) {
            ...
            if (names == null) {
                ...
                for(int i = 0; i < strSizeLimit.length; ++i) {
                    ...
                    VerifyAspect.SizeLimit limit = this.resolvePattern(pattern);
     ...
     * @param data
     */
    private void checkNumberLimit(VerifyAspect.SizeLimit limit, JSONObject data) {
     ...
     * @param data
     */
    private void checkSize(VerifyAspect.SizeLimit limit, JSONObject data) {
        Object value = this.valueOfPattern(limit.name, data);
        if (value == null) {
            throw new ParameterException(limit.name + " Can't be empty");
        } else {
     ...
     * @return
     */
    private VerifyAspect.SizeLimit resolvePattern(String pattern) {
        VerifyAspect.SizeLimit res = new VerifyAspect.SizeLimit();
